...
def LSTM_single():
    ...
    temp_res = subprocess.run("aws s3 ls --no-sign-request s3://noaa-goes18/ABI-L2-LSTM/{year}/{day}/{hour}/".format(year=year, day=day, hour=hour), shell=True, check=True, stdout=subprocess.PIPE, text=True)
    ...
    # print(file)
        subprocess.run("aws s3 cp --no-sign-request s3://noaa-goes18/ABI-L2-LSTM/{year}/{day}/{hour}/{file} files/single_files/LSTM/{year}/{day}/{hour}/{file}".format(year=year, day=day, hour=hour, file=file), shell=True, check=True)
